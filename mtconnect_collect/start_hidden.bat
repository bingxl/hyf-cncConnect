...
wscript "%HERE%tools\hidden_run.vbs" "cmd /c taskkill /F /IM agent.exe 2>nul & taskkill /F /IM fanuc_adapter.exe 2>nul & taskkill /F /IM mazak_adapter.exe 2>nul & taskkill /F /IM mtc_stats.exe 2>nul & taskkill /F /IM webserver.exe 2>nul & taskkill /F /IM cnc_sim.exe 2>nul & taskkill /F /IM shdr_sim.exe 2>nul & taskkill /F /IM mazak_sim.exe 2>nul"
...
for /f "usebackq tokens=1-5" %%t in ("%HERE%agent\adapters.txt") do (
    if "%%t"=="FANUC" (
        ...
        wscript "%HERE%tools\hidden_run.vbs" "%HERE%bin\cnc_sim.exe %%x !CTLPORT! 500 %%~u > %HERE%log\%%u.log 2>&1"
    )
)
...
REM --- stats poller ---
wscript "%HERE%tools\hidden_run.vbs" "cmd /c cd /d %HERE% && %HERE%bin\mtc_stats.exe stream 5000 stats.db 5000 > %HERE%log\stats_poll.log 2>&1"
